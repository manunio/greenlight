...
// receiver (our Runtime type), we use a pointer receiver for this to work
// correctly. Otherwise, we will only be modifying a copy (which is them discarded when
// this method returns).
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	...
	// double-quotes from the string, If we can't unquote it, then we return the
	...
	// If it isn't, we return ErrInvalidRuntimeFormat error again
	if len(parts) != 2 || parts[1] != "mins" {
		return ErrInvalidRuntimeFormat
	}

	// Otherwise, parse the string containing the number to int32, Again, if this
	...
	// type) in order to set underlying value of the pointer.
	*r = Runtime(i)
