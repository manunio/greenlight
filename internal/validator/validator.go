...
// Declare a regular expression for sanity checking the format of email address (we'll
...
// AddErrors add an error message to the map (so long as no entry already exists for
// the given key).
...
// Matches returns true if a string values in a slice are unique.
